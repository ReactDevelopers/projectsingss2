...
@section('content-header')
    <h1>
        {{ trans('category::subcategories.title.subcategories') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li class="active">{{ trans('category::subcategories.title.subcategories') }}</li>
    </ol>
@stop
@section('styles')
        ...
        <div class="col-xs-12">
            <div class="row">
                <div class="btn-group pull-right" style="margin: 0 15px 15px 0;">
                    <a href="{{ route('admin.category.subcategory.create') }}" class="btn btn-primary btn-flat" style="padding: 4px 10px;">
                        <i class="fa fa-pencil"></i> {{ trans('category::subcategories.button.create subcategory') }}
                    </a>
                </div>
            </div>
            <div class="box box-primary">
                ...
                <div class="box-body">
                    {{-- <div class="row">
                    <table border="0" cellspacing="5" cellpadding="5" margin="10px" style="margin-bottom: 10px; margin-right: 15px;float: right;">
                        <tbody><tr>
                                <td>{{ trans('category::categories.title.categories') }}:&nbsp;</td>
                                <td>
                                    {!! Form::select("category_id", $categories, old("category_id"), ['class' => "form-control form-search-parent-category", 'id' => "category_id",  'name' => "category_id"]) !!}
                                </td>
                            </tr>
                        </tbody>
                    </table>    
                    ...
                    <table class="data-table table table-bordered table-hover">
                        ...
                        <tr>
                            ...
                            <th>
                                <a class="header-table" href="{{ get_url_query($current_url, $query_url, ['order_field' => 'name', 'sort' => $order_field == "name" ? ($sort == "DESC" ? "ASC" : "DESC") : "DESC"]) }}" >
                            ...
                            {{-- <th>{{ trans('category::subcategories.table.parent category') }}</th> --}}
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group">
                                    <a href="{{ route('admin.category.subcategory.edit', [$item->id]) }}" class="btn btn-default btn-flat"><i class="fa fa-pencil"></i></a>
                                    <button type="button" class="btn btn-danger btn-flat" data-toggle="modal" data-target="#modal-delete-confirmation" data-action-target="{{ route('admin.category.subcategory.destroy', [$item->id]) }}"><i class="fa fa-trash"></i></button>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-7" style="text-align: right">
                            {!! $subcategories->appends(['limit' => $limit, 'keyword' => $keyword, 'page' => $page])->render() !!}
                        </div>
                    </div>
                    {!! Form::close() !!}
                </div>
...
@section('shortcuts')
    <dl class="dl-horizontal">
        <dt><code>c</code></dt>
        <dd>{{ trans('category::subcategories.title.create subcategory') }}</dd>
    </dl>
@stop
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            $(document).keypressAction({
                actions: [
                    { key: 'c', route: "<?= route('admin.category.subcategory.create') ?>" }
                ]
            });
        });
    </script>
    ...
    <script type="text/javascript">
        ...
        /*$.fn.dataTable.ext.search.push(
         ...
         $(document).ready(function() {
            ...
            $(".form-search-parent-category").select2();
