...
class CreateViewForMyPlacementAndSubordinatePlacement extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        DB::statement("CREATE VIEW `my_and_subordinate_placement` AS SELECT
            ...
            ON
                cr.id = placements.course_run_id and cr.should_check_deconflict = 'Yes'
            WHERE
                placements.deleted_at IS NULL AND placements.current_status = 'Confirmed'
            UNION ALL
            SELECT
                subordinate_p.id,
            ...
            JOIN course_runs AS cr
            ON
                cr.id = subordinate_p.course_run_id and cr.should_check_deconflict = 'Yes'
            JOIN users AS u
            ON
                u.personnel_number = subordinate_p.personnel_number AND u.supervisor_personnel_number IS NOT NULL AND u.deleted_at is NULL
            WHERE
                subordinate_p.deleted_at IS NULL AND subordinate_p.current_status = 'Confirmed';");
    }
