...
class CourseRunSummaryUploadTest extends TestCase
{
    ...
    public function test_upload_with_blank_request() {
        ...
        $response = $this->call('POST','course-run/upload-summary', [],[],[], $this->getAuthHeader(true));
    ...
    public function test_upload_with_invalid_file() {
        ...
        $response = $this->call('POST','course-run/upload-summary',[],[],['file'=> $file], $this->getAuthHeader());
        $this->assertResponseStatus(422);
    ...
    public function test_upload_with_valid_data() {
        ...
        $file_name = 'Course_run_summary.xlsx';
        $file = new UploadedFile(base_path('tests/Sample_files/'.$file_name),$file_name,null, null, null,true);        
        $response = $this->call('POST','course-run/upload-summary',[],[],['file'=> $file], $this->getAuthHeader());
        $this->assertResponseStatus(200);

        $courseRun->update(['deleted'=> null,'current_status'=> 'Confirmed','summary_uploaded'=>'No']);
