...
<div class="login-inner-wrapper profile-info-details">
    <div class="no-wrapper">
        ...
        <h2 class="form-heading bold-heading">
            {{ trans('website.W0485').' '. array_column($user['industry'],'name')[0]}} 
            <a href="{{url(sprintf('%s/profile/step/two',TALENT_ROLE_TYPE))}}" title="Edit">
                <img height="12" src="{{asset('images/edit-icon.png')}}" />
            </a>
        </h2>
        ...
        <div class="m-t-35">
            ...
            @if(isset($user['talentCompany']))
                <h6 class="">Current Employment: {{@$user['talentCompany']->company_name}} {{(@$user['notice_expired']!=null) ? '(On Notice Period - End on '.(@$user['notice_expired']).')': ''}}</h6>
            @endif
            <div class="form-group clearfix">
