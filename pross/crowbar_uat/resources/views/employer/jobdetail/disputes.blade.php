<div class="row mainContentWrapper">
    @includeIf('employer.jobdetail.includes.dispute',['talent' => $project->proposal->talent])
    <div class="col-md-8 col-sm-8 right-sidebar">
        <div>
            <ul class="user-profile-links">
                ...
                @endif
                <li class="active">
                    <a href="{{url('employer/project/dispute/details?job_id='.___encrypt($project->id_project))}}">
                        {{trans('website.W0409')}}
                    </a>
                </li>
            </ul>
            ...
            <div class="job-detail-final">
                <div class="login-inner-wrapper raise-dispute-section" style="padding: 20px;">
                    ...
                    @if(empty($project->dispute) || ($project->dispute->can_reply == DEFAULT_YES_VALUE && !empty($project->dispute->time_left)))
                        <form role="submit-raise-dispute" action="{{ url(sprintf('%s/project/submit/dispute',EMPLOYER_ROLE_TYPE)) }}" method="POST" class="form-horizontal">
                            {{ csrf_field() }}
                            @if(0)
                                <p class="heading-grey">
                                    @if(empty($project->dispute))
                                        {!!sprintf(trans('website.W0410'),$project->proposal->talent->name)!!}       
                                    @else
                            ...
                            @endif
                            <div class="review-form no-padding-bottom">
                                @if(empty($project->dispute))
                                    <div class="form-group">
                                        <div class="col-md-12 col-sm-12 col-xs-12">
                                            <div class="raise-dispute-select">
                                                <select class="form-control" name="reason">
                                                    {!! ___dropdown_options(___cache('dispute_concern'),trans('website.W0823'),'-1') !!}
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <form class="form-horizontal" role="raisedispute" action="{{url(sprintf('%s/document/raisedispute',EMPLOYER_ROLE_TYPE))}}" method="POST" accept-charset="utf-8">
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="row form-btn-set">
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <a href="{{ url(sprintf('%s/project/details?job_id=%s',EMPLOYER_ROLE_TYPE, ___encrypt($project['id_project']))) }}" class="button-line">{{ trans('job.J0084')}}</a>
                                </div>
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    ...
                                    <button type="button" class="hide" data-request="ajax-submit" data-target="[role='submit-raise-dispute']" value="{{trans('website.W0013')}}">{{ trans('website.W0013') }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
