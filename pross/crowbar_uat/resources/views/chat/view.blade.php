	...
	<div class="modal fade upload-modal-box" id="report-abuse" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
            <h3 class="modal-title bottom-margin-10px">{{ trans('website.W0350') }}</h3>
            <div class="modal-body bg-white">
                <div>
                	<p><b data-target="reported-user"></b></p>
	                <p class="heading-grey">{!!sprintf(trans('website.W0801'),url('page/terms-and-conditions'))!!}</p>
	                <br>
	                <form role="report-abuse" action="{{url(sprintf('%s/report-abuse',$user['type'])) }}" method="POST" accept-charset="utf-8">
	                    {{ csrf_field() }}
	                    <div class="form-group">
	                        <textarea rows="10" class="form-control" name="reason" placeholder="{{ trans('website.W0354') }}"></textarea>
            ...
            <div class="report-modal-footer">
	            <div class="button-group">
                    ...
                    <button type="button" style="padding: 10px 20px;" class="button" data-request="report-abuse">{{ trans('website.W0350') }}</button>
	            </div>
            </div>
        </div>
      </div>
...
@push('inlinescript')
	...
	<script>
		@if(!empty($request['receiver_id']))
			writeCookie('current_chat_window',"{{___decrypt($request['receiver_id'])}}");
		@endif
		...
		var report_abuse_company_text 	= '{{ trans("website.W0802") }}';
		var delete_all_text 			= '{{ trans("website.W0351") }}';
		var terminate_chat_text 		= '{{ trans("website.terminate_chat") }}';
		...
		var terminate_message_text 		= '{{ trans("website.terminate_message") }}';
		var delete_message_text 		= '{{ trans("website.W0620") }}';
		var attachement_text       		= '{{ trans("website.W0621") }}';
		var job_title_text       		= '{{ trans("website.jobid") }}';
		...
		var no_chat_list 			= "{!!sprintf(NO_CHAT_CONNECTION_TEMPLATE,trans('website.W0559'),trans('website.W0747'),url(sprintf('%s/find-jobs',TALENT_ROLE_TYPE)),trans('website.W0500'),trans('website.W0342'))!!}";
		@else
