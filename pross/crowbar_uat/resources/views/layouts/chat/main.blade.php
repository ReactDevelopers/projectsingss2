...
<html lang="en">
    <head>
        ...
        <link href="{{asset('css/placeholder.min.css')}}" rel="stylesheet" />
    ...
    <body>
        <div class="wrapper">
            @include(sprintf('%s.includes.%s',(\Auth::user()->type),$header))
            @yield('content')
            @include('footer')
            {{-- @include(sprintf('%s.includes.%s',(\Auth::user()->type),$footer)) --}}
        </div>
