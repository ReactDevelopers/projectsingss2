			...
			<div class="afterlogin-section viewProfile">
				<div class="container">
                    <div class="row mainContentWrapper">
						@includeIf('talent.profile.includes.sidebar')
							...
							<div class="navigation-bar">
								<ul>
									<li class="@if(in_array('one',$steps)) {{ 'selected'}} @endif">
										...
										<a href="{{ url(sprintf('%s/profile/%sstep/one',TALENT_ROLE_TYPE,$edit_url)) }}" class="navigation-dot"></a>
									</li>
									<li class="@if(in_array('two',$steps)) {{ 'selected'}} @endif">
										<span class="navigation-stepCount">2</span>
										<a href="{{ url(sprintf('%s/profile/%sstep/two',TALENT_ROLE_TYPE,$edit_url)) }}" class="navigation-dot"></a>
									</li>
									<li class="@if(in_array('three',$steps)) {{ 'selected'}} @endif">
										<span class="navigation-stepCount">3</span>
										<a href="{{ url(sprintf('%s/profile/%sstep/three',TALENT_ROLE_TYPE,$edit_url)) }}" class="navigation-dot"></a>
									</li>
									<li class="@if(in_array('four',$steps)) {{ 'selected'}} @endif">
										<span class="navigation-stepCount">4</span>
										<a href="{{ url(sprintf('%s/profile/%sstep/four',TALENT_ROLE_TYPE,$edit_url)) }}" class="navigation-dot"></a>
									</li>
									<li class="@if(in_array('five',$steps)) {{ 'selected'}} @endif">
										<span class="navigation-stepCount">5</span>
										<a href="{{ url(sprintf('%s/profile/%sstep/five',TALENT_ROLE_TYPE,$edit_url)) }}" class="navigation-dot"></a>
									</li>
								</ul>
							</div>
							@includeIf($view)
