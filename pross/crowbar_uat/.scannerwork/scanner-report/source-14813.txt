    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body bg-white">
                <div>
                    <p><b>{{ sprintf(trans('website.W0410'),$job_details['accepted_talent_name']) }}</b></p>
                    <p>{{ trans('website.W0411') }}</p>
                    <br>
                    <form role="add-raise-dispute" method="POST" action={{ url(sprintf('%s/raise/dispute?project_id=%s',EMPLOYER_ROLE_TYPE,___encrypt($job_details['id_project']))) }}> 
                        {{ csrf_field() }}
