            ...
            <div class="row mainContentWrapper">
                ...
                <div class="col-md-3 left-sidebar bottom-margin-10px" id="left-sidebar">
                    <div class="sidebar-content">
                        <h3>Filters <span data-request="clear-filter" data-url="{{ url(sprintf('%s/find-jobs',TALENT_ROLE_TYPE)) }}" id="clearAll">Clear All</span></h3>
                        <div class="filter-options">
                            <h4>{{ trans('website.W0342') }}</h4>
                            <div class="filter-list-group">
                                <input type="text" value="{{ \Request::get('_search') }}" name="search" placeholder="Search" class="form-control" id="search_job" data-request="search"/>
                            </div>
                            ...
                            <ul class="filter-list-group">
                                @foreach(employment_types('web_post_job') as $key => $value)
                                    <li>
                                        <div class="checkbox ">                
                                            <input type="checkbox" id="employement-{{ $value['type'] }}" name="employment_type_filter" value="{{ $value['type'] }}"  data-action="filter" @if(in_array($value['type'],$user['interested']) && 0) checked="checked" @endif>
                ...
                <div class="col-md-9 right-sidebar">
                    ...
                    <div>
                       <div id="loadmore">
                           <button type="button" class="btn btn-default btn-block btn-lg hide" data-request="filter-paginate" data-url="{{ url(sprintf('%s/_find-jobs',TALENT_ROLE_TYPE)) }}" data-target="#job_listing" data-showing="#paginate_showing" data-loadmore="#loadmore" data-form="[role='find-jobs']">{{ trans('website.W0254') }}</button>
                       </div>
                   </div>
