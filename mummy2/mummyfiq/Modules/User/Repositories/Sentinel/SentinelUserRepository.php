...
class SentinelUserRepository implements UserRepository
{
    ...
    public function delete($id)
    {
        if ($user = $this->user->find($id)) {
            if($user->hasRoleId(Config('constant.user_role.customer'))){
                $customerService = app(CustomerService::class);
                return $customerService->destroy($user);
            }elseif($user->hasRoleId(Config('constant.user_role.vendor'))){
                $vendorService = app(VendorService::class);
                return $vendorService->destroy($user);
            }else{
                $now = Carbon::now()->timestamp;

                if(!empty($user->facebook_id)) $user->facebook_id = $user->facebook_id . '_' . $now;
                if(!empty($user->google_id)) $user->google_id = $user->google_id . '_' . $now;
