        ...
        <?php if (isset($$zoneVar)): ?>
            <?php foreach ($$zoneVar as $file): ?>
                <figure data-id="{{ $file->id }}">
                    <img src="{{ Imagy::getThumbnail($file->media_url, (isset($thumbnailSize) ? $thumbnailSize : 'largeThumb')) }}" alt="" width="180px"/>
                    <a href="#" data-href="" class="delete-dialog" data-toggle="modal" data-target="#modal-confirm-delete">
                        <i class="fa fa-times-circle removeIcon"></i>
                    </a>
<!--                     <a class="jsRemoveLink" id="jsRemoveLink" href="#" data-id="{{ $file->id }}">
                        <i class="fa fa-times-circle removeIcon"></i>
                    </a> -->
                </figure>
            <?php endforeach; ?>
    ...
    $( document ).ready(function() {
        ...
        $('#jsRemoveLink').on('click', function (e) {
            ...
            if ("" != imageableId) {
                $.ajax({
                    type: 'POST',
                    url: '{{ route('api.portfolio.unlink') }}',
                    data: {
